...
const CreateCompany = () => {
  const { companies, getCompany } = useProductContext();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(`${API}/api/company/create`, {
        name,
      });
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${API}/api/company/update/${selected._id}`,
        { name: updatedName }
      );
      if (data?.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(`${API}/api/company/delete/${pId}`);
      if (data?.success) {
